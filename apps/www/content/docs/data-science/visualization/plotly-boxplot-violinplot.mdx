...
image: https://imrichard.com/wp-content/uploads/2025/04/plotly-boxplot-violinplot-distribution-cover.avif
---
...
![并排对比展示员工年龄分布的箱线图（蓝色）与小提琴图（红色）](https://imrichard.com/wp-content/uploads/2025/04/age-distribution-boxplot-violin-comparison.avif)
...
![展示员工年龄分布的小提琴图，图中用文字注释标明了中位数、上下四分位数、最大值和最小值](https://imrichard.com/wp-content/uploads/2025/04/annotated-age-distribution-violin-plot.avif)

这样处理后，即使是静态图片，也能清晰地看到数据的关键统计摘要信息，方便阅读和理解。

## 总结

相比于单一的平均值，箱线图和小提琴图能够提供更丰富的数据分布信息，帮助我们更准确地把握数据的形态、离散情况和潜在的异常点。Plotly 库为绘制这两种图表提供了便捷的方法，并且支持将它们组合展示或添加自定义注释，以满足不同的分析和展示需求。在日常数据分析工作中，适时运用这些可视化工具，有助于我们做出更可靠的判断和洞察。
