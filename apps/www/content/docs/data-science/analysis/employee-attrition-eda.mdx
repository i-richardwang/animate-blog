...
<Tabs defaultValue="plotly">
  ...
  <TabsContents>
    <TabsContent value="plotly">
      ![Plotly 绘制的 Attrition 标签分布柱状图与饼图，显示类别不平衡。](https://imrichard.com/wp-content/uploads/2025/04/eda_label_distribution_plotly.avif)
    ...
    <TabsContent value="matplotlib">
      ![Matplotlib 绘制的 Attrition 标签分布柱状图与饼图，显示类别不平衡。](https://imrichard.com/wp-content/uploads/2025/04/eda_label_distribution_matplotlib.avif)
...
<Tabs defaultValue="plotly">
  ...
  <TabsContents>
    <TabsContent value="plotly">
      ![Plotly 绘制的各特征与 Attrition 的相关性系数条形图](https://imrichard.com/wp-content/uploads/2025/04/eda_correlation_coefficient_plotly-scaled.avif)
    ...
    <TabsContent value="matplotlib">
      ![Matplotlib 绘制的各特征与 Attrition 的相关性系数条形图](https://imrichard.com/wp-content/uploads/2025/04/eda_correlation_coefficient_matplotlib-scaled.avif)
...
<Tabs defaultValue="plotly">
  ...
  <TabsContents>
    <TabsContent value="plotly">
      ![Plotly 绘制的 Age 在不同 Attrition 状态下的直方图与KDE曲线](https://imrichard.com/wp-content/uploads/2025/04/eda_continuous_feature_distribution_plotly.avif)
    ...
    <TabsContent value="matplotlib">
      ![Matplotlib 绘制的 Age 在不同 Attrition 状态下的直方图与KDE曲线](https://imrichard.com/wp-content/uploads/2025/04/eda_continuous_feature_distribution_matplotlib.avif)
...
<Tabs defaultValue="plotly">
  ...
  <TabsContents>
    <TabsContent value="plotly">
      ![Plotly 绘制的按 Department 分组的员工数与流失率](https://imrichard.com/wp-content/uploads/2025/04/eda_discrete_feature_distribution_plotly.avif)
    ...
    <TabsContent value="matplotlib">
      ![Matplotlib 绘制的按 Department 分组的员工数与流失率](https://imrichard.com/wp-content/uploads/2025/04/eda_discrete_feature_distribution_matplotlib.avif)
...
<Tabs defaultValue="plotly">
  ...
  <TabsContents>
    <TabsContent value="plotly">
      ![Plotly 绘制的 DistanceFromHome 在不同 MaritalStatus 与 Attrition 组合下的箱线图比较。](https://imrichard.com/wp-content/uploads/2025/04/eda_feature_combination_boxplot_plotly.avif)
    ...
    <TabsContent value="matplotlib">
      ![Matplotlib 绘制的 DistanceFromHome 在不同 MaritalStatus 与 Attrition 组合下的箱线图比较。](https://imrichard.com/wp-content/uploads/2025/04/eda_feature_combination_boxplot_matplotlib.avif)
...
![按 MaritalStatus 分别绘制的 DistanceFromHome KDE曲线](https://imrichard.com/wp-content/uploads/2025/04/eda_feature_combination_kde_plotly.avif)
...
![Plotly 绘制的 Age & MonthlyIncome 与标签的等高线图](https://imrichard.com/wp-content/uploads/2025/04/eda_feature_combination_contour_plotly.avif)
...
通过本阶段的探索性数据分析（EDA），我们：

1. 确认了目标变量 `Attrition` 存在显著的**类别不平衡**问题，需要在后续建模中妥善处理。
2. 通过相关性分析，初步识别了一批与流失**强相关**的特征（正向和负向），如 `OverTime`, `TotalWorkingYears`, `Age`, `MonthlyIncome` 等，为特征选择提供了线索。
3. 深入分析了关键**连续特征**（如 `Age`）和**离散特征**（如 `Department`）在不同流失状态下的分布差异和流失率，发现了具体的模式（如年轻员工流失率高、特定部门流失率高），为**特征工程**（如分箱、创建阈值特征、编码）提供了直接依据。
4. 探索了**特征组合**（如 `MaritalStatus` & `DistanceFromHome`, `Age` & `MonthlyIncome`）对流失的影响，揭示了**交互效应**的存在，为构建更精细化的特征或选择合适的模型提供了思路。

这些基于 EDA 的发现，不仅加深了我们对员工流失现象背后驱动因素的理解（实现了部分"**Insight**"目标），也为下一阶段的**特征工程**奠定了坚实的基础。只有将这些从数据中提炼出的模式和洞察转化为有效的模型输入（特征），我们才能构建出既准确（**Prediction**）又具有解释力的员工流失预测模型。

在下一篇文章中，我们将正式进入**特征工程**阶段，基于本次 EDA 的发现和业务理解，系统地创建、转换和选择特征，为模型训练做好最终准备。
