...
![员工流失预测项目训练集数据列信息统计表，展示各字段的缺失值、唯一值数量及数据类型。](/images/docs/data-science/employee-attrition-train-data-column-info-summary.webp)
...
![训练数据集重复行检测统计，显示重复记录数量为0，占比为0%。](/images/docs/data-science/employee-attrition-train-duplicate-rows-check.webp)
...
![测试集中发现训练集未包含的类别特征值列表](/images/docs/data-science/employee-attrition-test-new-categorical-values.webp)
...
![使用三倍标准差法检测出的各数值字段异常值数量统计列表](/images/docs/data-science/employee-attrition-train-outlier-counts-summary.webp)
...
![DailyRate字段箱线图和直方图分析](/images/docs/data-science/visualize_outliers_DailyRate.webp)

- `Education`：出现值为15的异常点，而其他编码都在1-5范围内，这很可能是异常值。

![Education字段箱线图和直方图分析](/images/docs/data-science/visualize_outliers_Education.webp)

- `JobLevel`：出现值为7的点，如果职级体系中不存在7级，则属于异常值。

![JobLevel字段箱线图和直方图分析](/images/docs/data-science/visualize_outliers_JobLevel.webp)
