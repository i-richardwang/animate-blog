...
![Datalore 的主工作空间界面，展示了 Notebook 列表和左侧导航菜单。](/development/self-hosted/datalore-workspace-dashboard-notebooks-scaled.webp)
...
![Datalore Notebook 中用于插入交互式控件的菜单，包含下拉框、滑块等选项。](/development/self-hosted/datalore-notebook-insert-interactive-elements.webp)
...
![Datalore 环境管理界面，显示已安装的 Python 库 plotly 及其版本和信息。](/development/self-hosted/datalore-environment-management-plotly-scaled.webp)
...
![Datalore 交互式下拉框控件的配置界面，展示设置标签、变量名和选项值。](/development/self-hosted/datalore-interactive-dropdown-config.webp)
...
![Datalore 的报告构建器界面，用于选择单元格创建分享报告，并实时预览包含交互控件和图表的最终效果。](/development/self-hosted/datalore-report-builder-sharing-preview-scaled.webp)

发布后，访问分享链接会看到一个简洁的页面。用户可以直接操作下拉框来切换图表类型，并实时看到结果，而无需接触或理解背后的 Python 代码。

下面的动图展示了最终分享页面的交互效果：

![Datalore 分享报告的交互效果动图演示：用户通过下拉框选择不同图表类型，图表随之动态更新](/development/self-hosted/datalore-shared-interactive-report-demo.gif)
