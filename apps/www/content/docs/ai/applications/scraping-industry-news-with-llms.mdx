...
![浏览器开发者工具中显示的晚点LatePost网站新闻标题HTML结构，突出显示headlines-title类的定位。](/ai/applications/latepost-inspect-news-title-html-scaled.webp)
...
这个流程把三个环节串联起来：用 Selenium 抓取动态网页的新闻链接，用 Jina Reader 提取正文内容，用大模型分析并提取关键信息（如组织变动、人事任免）。相比人工逐篇阅读和摘要，这种自动化方式在效率和覆盖面上都有明显优势。

但要在实际业务中应用，还有几个现实问题需要解决：

**爬虫的稳定性**是第一关。网站结构经常变化，爬虫脚本需要定期维护。同时要遵守 robots.txt 协议，控制好请求频率，避免对目标网站造成负担。

**内容提取的准确性**也很关键。Jina Reader 对大部分网页效果不错，但遇到特别复杂的页面结构时，可能还需要定制化的解析规则。

**Prompt 的持续优化**直接影响信息提取的质量。根据实际效果迭代 Prompt，才能提高准确度和召回率。

**成本控制**不能忽视。调用大模型 API 有费用，需要根据预算选择合适的模型和调用频率。

最后，**信息可信度**需要人工把关。自动化流程获取的信息可以作为线索，但在用于重要决策前，最好还是人工核实一遍。

技术在进步，爬虫+大模型的组合在企业情报、市场研究等领域确实有应用前景。关键是找到适合自己场景的平衡点。
