...
<Callout type="info">
  ...
  让机器捕捉文本关联性的核心技术](https://richardwang.me/understanding-vector-embedding/)》。
</Callout>
...
RAG 的基础流程包括五个环节：加载文档、分片、向量化、检索、生成答案。在处理财报这类特定文档的问答时，相比手动翻阅，效率和准确性都有提升。关键是让 LLM 基于检索到的原文来回答，而不是"自由发挥"。

实际应用中会遇到很多挑战：

**文档处理的复杂性。** 真实的 PDF 不只有文字，还有表格、图表、多栏布局。如何正确提取这些内容，是第一关。

**问题理解的精准度。** 用户的问题可能很模糊（"公司业绩如何？"），或者需要跨文档推理。这时简单的向量检索可能不够用。

**检索质量的提升空间。** 基础的向量检索有时会漏掉关键信息。可以尝试查询转换（把一个问题分解成多个子问题）、混合检索（结合向量和关键词）、重排序（二次筛选最相关的片段）等技术来改进。

**Prompt 的持续优化。** 如何引导 LLM 更好地利用检索到的上下文，如何处理检索结果不足的情况，这些都需要不断迭代 Prompt。

从基础 RAG 到生产级系统，需要根据具体场景不断优化。但掌握这个核心流程，是解决文档问答问题的第一步。
